...
import gr.manousos.iot.parser.GreekDateTimeParser;
...
public class SampleServiceImpl implements SampleService {
	...
	@Autowired
	GreekDateTimeParser greekDateTimeParser;
	...
	public boolean saveSample(SensorData sample) {
		...
		if (sample.getName().equals("DHT22")) {
			...
			t.setSampleDate(greekDateTimeParser.convertCurrentLocalDateTimeToGreek());
			t.setTemperature(sampleParser.parseTemperature());
			t.setLocation(loc);

			h.setSampleDate(greekDateTimeParser.convertCurrentLocalDateTimeToGreek());
